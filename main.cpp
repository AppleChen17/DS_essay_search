...
int filecount = 0;
const int THREADS_PER_BATCH = 10;
...
vector<string> get_file_list(const string& data_dir)
{
...
void construct_trie(string& filename, vector<Trie> trie)
{
	ifstream fi;
	...
	Trie trie1 = Trie();
	Trie trie2 = Trie();
	trie.push_back(trie1);
	trie.push_back(trie2);
...
// 批次平行讀檔 !!!
void process_files_in_batches(const vector<string>& fileList, vector<vector<Trie>>& trie) 
{
    int filecount = fileList.size(); // Assuming this is the total file count
    int batch_count = (filecount + THREADS_PER_BATCH - 1) / THREADS_PER_BATCH; 

    for (int batch = 0; batch < batch_count; batch++) 
    {
        vector<std::thread> batch_threads;

        // batch range
        int start = batch * THREADS_PER_BATCH;
        int batch_end = std::min(start + THREADS_PER_BATCH, filecount);

        for (int i = start; i < batch_end; i++) 
        {
            batch_threads.push_back(std::thread(construct_trie, fileList[i], std::ref(trie[i])));
        }
...
int main(int argc, char *argv[])
{
	...
	vector<string> fileList = get_file_list(data_dir);
	cout << "filecount = " << filecount << "\n";
	trie.assign(filecount+2, vector<Trie>());

	process_files_in_batches(fileList, trie);
